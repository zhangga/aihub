...
`long-run-harness` 是一个用于长周期任务的仓库级持久执行协议。现在 Codex 已有 goal 模式，它的目标不是替代 goal 模式、coding agent 或 shell runner，而是在项目内留下可提交、可审计、可恢复的任务状态，让 Agent 在多轮会话、上下文压缩或中断后仍能接上工作。
...
- 作为 Codex goal 模式的补充，在仓库内保存长任务的结构化状态和恢复线索。
- 支持 Codex 或类似 Agent 在需要持久状态的长任务中持续推进，而不是完成一个局部步骤后停止。
...
- 不替代 Codex goal 模式；如果运行时目标追踪已经足够，不需要启用该 skill。
